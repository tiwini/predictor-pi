...
def test_adaptive_lambda_extreme_tail_moves_toward_min():
    lam = p.adaptive_lambda(0.05, 0.01, lambda_min=0.90,
                            lambda_max=0.99, a=1.5, b=1.0)
    assert 0.90 <= lam <= 0.99
    assert lam < 0.93


def test_adaptive_lambda_zero_return_is_finite():
    lam = p.adaptive_lambda(0.0, 0.01, lambda_min=0.90,
                            lambda_max=0.99, a=0.5, b=1.0)
    assert 0.90 < lam < 0.99


def test_ewma_sigma_adaptive_too_few():
    assert p.ewma_sigma_adaptive([0.01, -0.01]) == 0.0


def test_ewma_sigma_adaptive_constant_magnitude():
    rets = [0.01] * 100
    sigma = p.ewma_sigma_adaptive(rets)
    assert sigma == pytest.approx(0.01, abs=1e-6)


def test_ewma_sigma_adaptive_responds_more_to_tail_than_fixed():
    rets = [0.01] * 50 + [0.08]
    fixed = p.ewma_sigma(rets, lam=0.985)
    adaptive = p.ewma_sigma_adaptive(rets, lambda_min=0.90,
                                     lambda_max=0.985, a=1.5, b=1.0)
...
def test_ast_hour_uses_fixed_utc_minus_4():
    # 1970-01-01 04:00:00 UTC == 1970-01-01 00:00:00 AST
    assert p.ast_hour(4 * 3600) == 0
    assert p.ast_hour(17 * 3600) == 13


def test_hourly_drift_z_uses_made_at_hour_ast():
    assert p.hourly_drift_z(17 * 3600) == pytest.approx(
        p.HOUR_DRIFT_Z_BY_AST[13]
    )


def test_positive_drift_raises_quantile_and_prob_above_now():
    base = p.Prediction(symbol="BTCUSDT", now_price=50_000.0,
                        sigma_1m=0.001, sigma_horizon=0.01,
                        horizon_min=60, n_candles=500)
    drifted = p.Prediction(symbol="BTCUSDT", now_price=50_000.0,
                           sigma_1m=0.001, sigma_horizon=0.01,
                           horizon_min=60, n_candles=500, drift_z=0.20)

    assert p.quantile(drifted, 0.70) > p.quantile(base, 0.70)
    assert p.prob_above(drifted, 50_000.0) > p.prob_above(base, 50_000.0)


def test_negative_drift_lowers_quantile_and_prob_above_now():
    base = p.Prediction(symbol="BTCUSDT", now_price=50_000.0,
                        sigma_1m=0.001, sigma_horizon=0.01,
                        horizon_min=60, n_candles=500)
    drifted = p.Prediction(symbol="BTCUSDT", now_price=50_000.0,
