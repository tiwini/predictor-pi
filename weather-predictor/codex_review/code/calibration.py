...
def record_ext_signal(station_id: str, target_date: date,
    ...
    INSERT OR IGNORE: solo persiste el primer write del día (la señal con la
    que se toman decisiones matutinas, no la última de la tarde). Si
    ext_shift_info es None o no aporta ext_med, no escribimos nada.
    """
    ...
    try:
        c.execute(
            """INSERT OR IGNORE INTO daily_ext_signal
