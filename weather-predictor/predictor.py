...
# Typical daily-peak window (inclusive-exclusive, local hour). Used both
# for adaptive polling (predictor_web) and for per-hour σ in the Bayesian
# reweight: obs inside this window get smaller σ (more diagnostic of the
# daily max) than early-morning obs.
PEAK_HOURS: dict[str, tuple[int, int]] = {
    "KPHX": (14, 17),
    "KLAX": (12, 15),
    "KLAS": (14, 17),
    "KLGA": (13, 16),
...
def _cached(fn):
    def wrapped(station: Station, *a, **kw):
        ...
        if hit is not None and now - hit[0] < _FETCH_TTL:
            return hit[1]
        val = fn(station, *a, **kw)
...
fetch_current = _cached(fetch_current)
fetch_today_obs = _cached(fetch_today_obs)
fetch_precip_ensemble = _cached(fetch_precip_ensemble)
fetch_ensemble = _cached(fetch_ensemble)
